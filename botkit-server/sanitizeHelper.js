...
function fallbackSearch(userMsg, mode) {
  ...
  switch (mode) {
    case 1:
      return findSKU(userMsg);
      break;
    case 2:
      return findPartNumber(userMsg);
      break;
    case 3:
      return findMatching(userMsg);
      break;
    default:
      return findMatching(userMsg);
      break;
...
function extractIdentifier(userMsg) {
  ...
  if (finalMsg.search("PART") !== -1 || finalMsg.search("NUMBER") !== -1) {
    //search For Part Only
    return findPartNumber(finalMsg);
  } else if (finalMsg.search("SKU") !== -1) {
    //search For SKU Only
    return findSKU(finalMsg);
  } //search For any matching ID
  else {
...
function generateSkuQuery(utterance) {
  ...
  for (i = 0; i < activationKeywords.length; i++) {
    if (
      utterance.toLowerCase().search(activationKeywords[i].toLowerCase()) != -1
    )
      return activationObject[activationKeywords[i]] + " " + identifier;
  }
  return identifier;
}
...
function extractSkuId(userMsg) {
  const finalMsg = userMsg.toUpperCase().replace(/\s|-/g, "");

  for (let sku of allNumbers) {
    sku = sku.toString();
    if (finalMsg.includes(sku)) {
      if (finalMsg === sku) return sku;
      replacedMsg = replaceSkuId(sku, userMsg);
      return replacedMsg;
    }
  }
...
function replaceSkuId(skuId, userInput) {
  userInput = removeExtraSpaces(userInput);
  const lowercaseSkuId = skuId.toLowerCase();
  const lowercaseUserInput = userInput.toLowerCase();
  const skuIdLength = skuId.length;

  let matchedStr = "";
  let size = 0;

  for (let i = 0; i < lowercaseUserInput.length && size < skuIdLength; i++) {
    const char = lowercaseUserInput[i];

    if (char === lowercaseSkuId[size]) {
      matchedStr += char;
      size++;

      if (size === skuIdLength) {
        return lowercaseUserInput.replace(matchedStr, skuId.toUpperCase());
      }
    } else if (char === " " && matchedStr.length > 0) {
      matchedStr += " ";
    } else {
      matchedStr = "";
      size = 0;
...
function removeExtraSpaces(str) {
  ...
  for (let c of str) {
    ...
    if (isWhitespace(c) && temp[temp.length - 1] !== " ") {
      temp += " ";
    }
  }
  let modifiedStr = temp.replace(/ {2,}/g, " ");
